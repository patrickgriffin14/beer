class FavouritesController < ApplicationController
  ...
  def destroy
    @favourite.destroy if @favourite.id == session[:cart_id]
    session[:cart_id] = nil
    respond_to do |format|
      format.html { redirect_to catalog_url, notice: 'Yout favourites have been cleared.' }
      format.json { head :ok }
    end
  end

  private
    def invalid_favourite
    logger.error "Attempt to access invalid favourite #{params[:id]}"
    redirect_to catalog_url, notice: 'Invalid favourite'
    end    
